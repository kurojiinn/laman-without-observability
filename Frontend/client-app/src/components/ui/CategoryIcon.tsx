...
export type StoreCategoryType = "FOOD" | "PHARMACY" | "BUILDING" | "HOME" | "CLOTHES" | "AUTO";
...
const icons: Record<string, React.ReactNode> = {
  ...
  FOOD: (
    <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.8"
      strokeLinecap="round" strokeLinejoin="round" className="w-full h-full p-[22%]">
  ...
  PHARMACY: (
    <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"
      strokeLinecap="round" strokeLinejoin="round" className="w-full h-full p-[20%]">
  ...
  CLOTHES: (
    <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.8"
      strokeLinecap="round" strokeLinejoin="round" className="w-full h-full p-[18%]">
...
export default function CategoryIcon({ type, size = "md", className = "" }: CategoryIconProps) {
  const meta = CATEGORY_META[type] ?? DEFAULT_META;
  return (
    <div className={`${meta.bg} ${sizeClass[size]} flex items-center justify-center flex-shrink-0 ${className}`}>
      {meta.icon}
    </div>
  );
}
